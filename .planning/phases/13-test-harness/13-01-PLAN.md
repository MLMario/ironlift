...
- `setupFilesAfterEnv: ['<rootDir>/jest-setup.ts']` (runs after Jest environment is ready)
...
IMPORTANT: The correct Jest config key is `setupFilesAfterEnv` (NOT `setupFilesAfterSetup`). Using the wrong key will cause Jest to silently ignore the setup file and all mocks will be skipped.
...
Verify jest.config.ts exists with preset, setupFilesAfterEnv, transformIgnorePatterns, and collectCoverageFrom.
