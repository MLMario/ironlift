...
- Update `handleBlur` to handle the new null return: if `parseTimeInput(editingText)` returns null, do NOT call onSecondsChange. This is the revert behavior per locked decision ("Unparseable input -> revert to previous value"). The component simply exits editing mode and the display reverts to the previous `seconds` prop value since onSecondsChange was never called.
- Keep everything else identical -- no other changes to RestTimerInline.
  </action>
  <verify>
Run `npx tsc --noEmit` to confirm no type errors. Verify that `src/lib/timeUtils.ts` exports all three functions. Verify that `RestTimerInline.tsx` no longer has local formatTime/parseTimeInput definitions and imports from `@/lib/timeUtils`. Verify that handleBlur does NOT call onSecondsChange when parseTimeInput returns null.
  </verify>
  <done>
timeUtils.ts exists with formatTime, parseTimeInput (returns null for unparseable), and clampSeconds. RestTimerInline imports from timeUtils and reverts on unparseable input (does not call onSecondsChange when parseTimeInput returns null). Template editor rest time input behavior preserved.
  </done>
</task>
...
1. Add a `pause()` method:
   ...
   - Keeps `timer.status` as `'active'` -- this is critical. The parent (RestTimerBar) uses the `isActive` prop (derived from timer.status === 'active') to know a timer was paused mid-countdown (it will see isActive=true but the countdown is frozen)
   - Freezes the remaining value in place: `setTimer(prev => prev.status === 'active' ? { ...prev, remaining } : prev)`
   - Result: after pause(), timer.status remains 'active', timer.remaining is frozen at the paused value, but no interval is ticking and no notification is scheduled
...
Run `npx tsc --noEmit` to confirm no type errors. Verify useRestTimer returns `pause` in its return object. Verify useWorkoutState returns `updateRestSeconds` and `getRestTimeChanges` in its return object. Verify TemplateSnapshot has `rest_seconds` field.
  ...
  <done>
useRestTimer exposes pause() that freezes timer.remaining at the paused value while keeping timer.status as 'active' (no interval ticking, no notification scheduled). useWorkoutState exposes updateRestSeconds(exerciseIndex, seconds) for session-scoped rest time changes and getRestTimeChanges() for dirty detection. TemplateSnapshot includes rest_seconds per exercise. No type errors.
  </done>
</task>
...
- RestTimerInline handleBlur does NOT call onSecondsChange when parseTimeInput returns null (revert behavior)
