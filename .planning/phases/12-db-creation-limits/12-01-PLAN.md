...
must_haves:
  truths:
    - "Inserting a 21st template for a user raises LIMIT_EXCEEDED:templates:20 with ERRCODE P0001"
    - "Inserting a 51st user-created exercise raises LIMIT_EXCEEDED:exercises:50 with ERRCODE P0001"
    - "Inserting a 26th chart for a user raises LIMIT_EXCEEDED:charts:25 with ERRCODE P0001"
    - "Inserting a 16th exercise into a template raises LIMIT_EXCEEDED:template_exercises:15 with ERRCODE P0001"
    - "Inserting a 16th exercise into a workout log raises LIMIT_EXCEEDED:workout_exercises:15 with ERRCODE P0001"
    - "Inserting an 11th set for a template exercise raises LIMIT_EXCEEDED:template_sets:10 with ERRCODE P0001"
    - "Inserting an 11th set for a workout log exercise raises LIMIT_EXCEEDED:workout_sets:10 with ERRCODE P0001"
...
| # | Table | Function | Trigger | Limit | Lock Key Prefix | Notes |
|---|-------|----------|---------|-------|-----------------|-------|
| 1 | `templates` | `enforce_max_templates` | `trg_enforce_max_templates` | 20 | `templates_` | Count WHERE user_id = NEW.user_id |
| 2 | `exercises` | `enforce_max_user_exercises` | `trg_enforce_max_user_exercises` | 50 | `exercises_` | Early return if NEW.is_system = true. Count WHERE user_id = NEW.user_id AND is_system = false |
| 3 | `user_charts` | `enforce_max_user_charts` | `trg_enforce_max_user_charts` | 25 | `charts_` | Count WHERE user_id = NEW.user_id |

**Per-parent triggers (count scoped to parent FK):**

| # | Table | Function | Trigger | Limit | Lock Key Prefix | Count WHERE |
|---|-------|----------|---------|-------|-----------------|-------------|
| 4 | `template_exercises` | `enforce_max_template_exercises` | `trg_enforce_max_template_exercises` | 15 | `tmpl_ex_` | template_id = NEW.template_id |
| 5 | `workout_log_exercises` | `enforce_max_workout_exercises` | `trg_enforce_max_workout_exercises` | 15 | `wk_ex_` | workout_log_id = NEW.workout_log_id |
| 6 | `template_exercise_sets` | `enforce_max_template_sets` | `trg_enforce_max_template_sets` | 10 | `tmpl_sets_` | template_exercise_id = NEW.template_exercise_id |
| 7 | `workout_log_sets` | `enforce_max_workout_sets` | `trg_enforce_max_workout_sets` | 10 | `wk_sets_` | workout_log_exercise_id = NEW.workout_log_exercise_id |

**ERRCODE:** All triggers use standard `P0001` (raise_exception). Specific limit identified by parsing the structured error message.

**Each function/trigger pair follows this pattern:**
```sql
-- N. Table: {table_name}
-- Limit: {limit} per {scope}
-- Error: LIMIT_EXCEEDED:{entity}:{limit} (ERRCODE: P0001)
...
9. All ERRCODE values are 'P0001' (standard raise_exception)
  </verify>
...
<task type="auto">
  ...
  <action>
Create `docs/error_documentation.md` documenting all 7 trigger limits.

**Structure:**
1. Title and purpose header
2. Quick reference table with all 7 limits (entity, table, limit, scope, error message)
3. Detailed section per limit including:
   - Entity name
   - Limit value
...
4. Section on how errors surface in the Supabase JS client:
   ...
   - Client identifies specific limit by parsing the message prefix: `error.message?.startsWith('LIMIT_EXCEEDED:')`
...
5. Test INSERT queries section -- one test per limit that can be run in Supabase SQL Editor to verify the trigger works. Each test:
   ...
   - Catches `WHEN raise_exception` and checks SQLERRM for the expected message
   - Cleans up test data
...
1. Contains quick reference table with all 7 limits (entity, table, limit, scope, message)
2. Each entry includes: entity, limit, function name, table, error message
